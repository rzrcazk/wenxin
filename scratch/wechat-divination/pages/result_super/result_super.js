...
const ai = require('../../utils/ai_super.js');
...
Page({
    ...
    onLoad(options) {
        ...
        // Skip animation logic for Super God Mode
        const skipAnimation = options.skip === 'true';
        if (skipAnimation) {
             this.setData({ step: 1 }); // Directly show content
        }
    ...
    onShareAppMessage() {
        if (this.data.isKnowledge) {
             return {
                ...
                path: `/pages/result_super/result_super?type=KNOWLEDGE&question=${encodeURIComponent(this.data.question)}`
            };
        }
        ...
        return {
            ...
            path: `/pages/result_super/result_super?lines=${linesStr}&question=${question}&complexity=${complexity}`
        };
    },
