...
const generateSummary = (question, hexagram, complexity = 'COMPLEX') => {
    ...
    const prompt = `作为一位精通周易的国学大师，请根据用户的问题${hasImage ? '、附带的图片' : ''}和卦象结果进行深度解读。
    用户问： "${question || '(用户未书写文字，请基于图片和卦象解读)'}"
    卦象：${hexagram.name} (${hexagram.description})
    
    请结合卦象的含义（卦辞、爻辞）以及用户的问题背景（包括图片内容，如果是手相/面相/风水图，请结合卦象进行关联分析），进行深入的思考和分析。
    **重要**：如果用户提供的卦象信息中缺少具体的“爻辞”（Ancient Line Text），请你务必在分析的开头补充引用该动爻的原文（文言文），并给予解释。
    ...
    回复要求：
    - 语气要专业、玄妙但又通俗易懂，不仅是翻译卦辞，要有深度思考的洞察。
    - 字数控制在200字左右。
    - 使用中文。`;
